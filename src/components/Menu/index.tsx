...
export default function Menu({ fixed, showMenu }: { fixed?: boolean, showMenu?: boolean }) {
    ...
    return (
        <div className={`${menu.open ? '' : 'hidden'} bg-[rgba(100,150,161,0.4)] backdrop-blur overflow-y-auto z-70 max-sm:pt-24 pt-28  max-sm:h-lvh max-mxs:w-11/12 max-sm:w-11/12 w-full flex flex-col items-center text-black   ${fixed ? 'fixed' : ''} ${menu?.open ? 'max-sm:block max-sm:fixed' : 'max-sm:hidden '}`} style={{ borderRight: '4px solid #E3E4E5' }}>
