...
const OrderModal = ({ isOpenModal, setIsOpenModal, orderSelectedList, setOrderSelectedList }: { isOpenModal: boolean, setIsOpenModal: any, orderSelectedList: any, setOrderSelectedList: any }) => {
    ...
    const [valuesModal, setValuesModal] = useState([
        ...
        {
            name: 'Maior quantidade de alunos',
            key: 5,
            selected: false
        },
    ...
    return (
        <div className="fixed bottom-0 z-100 left-0 right-0 flex items-end justify-end pointer-events-none" style={{ display: isOpen ? '' : 'none' }}>
            ...
            <div className={`max-sm:w-full max-lg:w-2/3 w-80/100 relative bottom-0 bg-white rounded-t-3xl pointer-events-auto flex items-center justify-center flex-col`} style={{ top: modalY }} onTouchStart={handleTouchStart} onMouseDown={handleMouseDown}>
                            ...
                            </div>
                            {
                                index < valuesModal.length - 1 ?
                                    <div className=' w-full flex items-center justify-center'>
                                        <hr className='w-full bg-lightGray  rounded-full' style={{ border: '2px solid #E3E4E5' }}/>
                                    </div>
                                    : null
                            }
                        </div>
